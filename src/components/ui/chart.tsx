...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  return (
    <style
      // This generates CSS variables per series key, with per-theme overrides.
...
// --- Tooltip content typing for Recharts v3 ---
type TTValue = number | string;
type TTName = number | string;

// Infer the element type of `payload` from the official content-props type:
type TooltipItem = NonNullable<
  Recharts.TooltipContentProps<TTValue, TTName>["payload"]
>[number];
...
function ChartTooltipContent({
  ...
  labelKey,
  ...divProps
}: Recharts.TooltipContentProps<TTValue, TTName> &
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [first] = payload as TooltipItem[];
...
// --- Legend content typing ---
type LegendItem = Recharts.LegendPayload & {
  color?: string;
...
function ChartLegendContent({
  ...
  nameKey?: string;
  payload?: LegendItem[];
  verticalAlign?: Recharts.LegendProps["verticalAlign"];
}) {
  const { config } = useChart();
